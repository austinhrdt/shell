...
connect () {
  local user="${2:-$NTID}"
  local url='https://'$GITHUB_TOKEN'@github.com/'$user'/'$1'.git'
...
connect-org () {
  ...
  local url='https://'$GITHUB_TOKEN'@github.com/'$org'/'$1'.git'
  git remote add origin $url
}
...
create-repo() {
  curl -ss -H "Authorization: token $GITHUB_TOKEN" --data '{"name":"'$1'"}' https://github.com/api/v3/user/repos >> /dev/null
}

# deletes a repo on github
# be careful  ... only deletes in user space
delete-repo() {
  curl -ss -X DELETE -u $NTID:$GITHUB_TOKEN https://github.com/api/v3/repos/$NTID/$1 
}

# clones all the repositories ha ...
clone-all() {
  curl -ss -H "Authorization: token $GITHUB_TOKEN" https://github.com/api/v3/users/$NTID/repos | \
  grep -w '"name":' | sed 's/.*://' | rev | cut -c 2- | rev | \
  xargs -L1 -I '$' git clone https://$GITHUB_TOKEN@github.com/$NTID/$.git
}
